var https = require('https')
  ...
  , setupReqRes = require('./util/https').setup
...
/**
 * Create an https server with the given options and create a `Router` to handle its requests.
 *
 * Options:
 *
...
Server.prototype.listen = function(port, host) {
  ...
  config.loadConfig('./', server, function(err, resourcesInstances) {
    if (err) {
      ...
      https.Server.prototype.listen.call(server, port || server.options.port, host || server.options.host);
    }
  });
  return this;
};
